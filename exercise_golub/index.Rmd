...
`mean_AML` and `mean_ALL` contain the mean of **log-transformed** (base 10) gene expression values. What if we wanted to make two new columns that contain antilogs for these values? Create two new columns called `geo_mean_AML` and `geo_mean_ALL` (because anti-logging the mean of log-transformed values is equal to the geometric mean of the original values) 
